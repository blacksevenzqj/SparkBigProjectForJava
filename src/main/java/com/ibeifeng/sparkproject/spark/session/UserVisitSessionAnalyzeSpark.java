 ...
 * 1、J2EE平台在接收用户创建任务的请求之后，会将任务信息插入MySQL的task表中，任务参数以JSON格式封装在task_param
 * 字段中
 * 2、接着J2EE平台会执行我们的spark-submit shell脚本，并将taskid作为参数传递给spark-submit shell脚本
 * 3、spark-submit shell脚本，在执行时，是可以接收参数的，并且会将接收的参数，传递给Spark作业的main函数
...
//		sessionid2actionRDD.checkpoint();
		 ...
		 * 		积累了，处理各种问题的经验
		 */
		// 获取top10热门品类：参数sessionid2detailRDD：通过筛选条件的session的访问明细数据RDD <sessionid, Row>
...
//		return actionDF.javaRDD().repartition(1000);
		...
		return actionRDD.mapPartitionsToPair(new PairFlatMapFunction<Iterator<Row>, String, Row>() {
			...
			public Iterable<Tuple2<String, Row>> call(Iterator<Row> iterator) throws Exception {
				...
				while(iterator.hasNext()) {
					...
					list.add(new Tuple2<String, Row>(row.getString(2), row));  // session_id
				}
				return list;
			}
		});
	...
	private static JavaPairRDD<String, String> aggregateBySession(
		 ...
		 * userid2PartAggrInfoRDD可能数据量还是比较大，比如，可能在1千万数据
		 * userid2InfoRDD可能数据量还是比较小的，你的用户数量才10万用户
		 */
		...
		// join之后的数据量和userid2PartAggrInfoRDD是相同的，也就是按session聚合的数据量。
...
//						// 将Broadcast（要join数据量小的RDD）转换为 Map格式
...
//						/**
//						 * 如果没有相应的tuple._1的key数据怎么办？
//						 * 1、当做left join处理？
//						 * 2、还是map结束之后还要接filter算子过滤掉未连接上的数据？
//						 */
...
//					}
//				});
//		// 4、采样数据reduceByKey后，反转key和value：做sortByKey降序排序
...
//					}
//				});
//		// 5、采样数据reduceByKey后 → 反转key和value → 做sortByKey降序排序 → 取出数据量最大的key
//		final Long skewedUserid = reversedSampledRDD.sortByKey(false).take(1).get(0)._2;
//		// 6.1、数据拆分：使用filter算子过滤出 数据量最大的key的RDD
...
//
//		/**
//		 * 7、原始业务中直接join的userid2InfoRDD：
//		 * 7.1、筛选出 数据倾斜的 采样出的 数据量最大的key
//		 * 7.2、随机生成100条数据（前缀打上随机数）
//		 */
...
//				});
//		/**
//		 * 8、数据倾斜的 采样出的 使用filter算子过滤出 数据量最大的key的RDD
//		 * 7.1、100以内的的随机数前缀
//		 * 7.2、100以内的的随机数前缀 join “7”中 随机生成100条数据（前缀打上随机数）
//		 * 7.3、剔除前缀
//		 */
...
//				});
		...
		/**
		 * 方案七：使用随机数和扩容表进行join（理解：将一个task打散到10个task中）
		 */
		// 1、userid2InfoRDD用flatMap进行扩容：将每条数据映射为多条数据，每个映射出来的数据都带了一个n以内的随机数，通常来说会选择10。
		...
		// 2、将userid2PartAggrInfoRDD做普通的map映射操作：每条数据都打上一个10以内的随机数。
	...
	private static void randomExtractSession(
		...
		// 开始实现我们的按时间比例随机抽取算法，总共要抽取100个session，先按照天数，进行平分
		// 100 / 有多少天  =  每天要抽取的数量
		...
		// 我们用flatMap算子，遍历所有的<dateHour,Iterable<aggrInfo>>格式的数据
		...
		JavaPairRDD<String, String> extractSessionidsRDD =
				time2sessionsRDD.flatMapToPair( // <dateHour, Iterable<aggrInfo>>
					...
					public Iterable<Tuple2<String, String>> call(Tuple2<String, Iterable<String>> tuple) throws Exception {
						...
						while(iterator.hasNext()) {
							...
							if(extractIndexList.contains(index)) {
								...
								// 将sessionid加入list：这样做是为了后续做join
		 ...
		 *  <sessionid, sessionid> join <sessionid, Row> = <sessionid, Tuple2<sessionid, Row>>
		 */
		JavaPairRDD<String, Tuple2<String, Row>> extractSessionDetailRDD = extractSessionidsRDD.join(sessionid2actionRDD);
		// 将Iterator迭代器传入匿名内部类，只foreach一次，就可以进行批量插入数据库操作。如果不传Iterator迭代器，就无法实现。
	...
	private static List<Tuple2<CategorySortKey, String>> getTop10Category(  
			long taskid,  
			JavaPairRDD<String, Row> sessionid2detailRDD) { // <sessionid, Row>
		/**
		 ...
		 * categoryidRDD中，是包含了所有的符合条件的session，访问过的品类id（3类ID）
		 * 上面分别计算出来的三份，各品类的点击、下单和支付的次数，可能不是包含所有品类的
		 * 比如，有的品类，就只是被点击过，但是没有人下单和支付
		 * 所以，这里，就不能使用join（内连接）操作，要使用leftOuterJoin操作，就是说，如果categoryidRDD不能
		 * join到自己的某个数据，比如点击、或下单、或支付次数，那么该categoryidRDD还是要保留下来的
		 * 只不过，没有join到的那个数据，就是0了
		 */
		// <categoryid, categoryid_click_count_str>
	...
	private static JavaPairRDD<Long, Long> getClickCategoryId2CountRDD(
		...
		/**
		 * 说明一下：
		 * 这里，是对sessionid2detailRDD的数据进行了filter过滤（没有使用上面flatMapToPair的数据）
		 * 点击行为的数据其实只占总数据的一小部分，所以过滤以后的RDD，每个partition的数据量会很不均匀，
		 * 而且数据量肯定会变少很多，所以针对这种情况，还是比较合适用一下coalesce算子的，在filter过后去减少partition的数量
		 * 对这个coalesce操作做一个说明：
		 ...
		 */
		JavaPairRDD<String, Row> clickActionRDD = sessionid2detailRDD.filter( // 返回true 或 false过滤
...
//				});
	...
	private static JavaPairRDD<Long, String> joinCategoryAndData(
			JavaPairRDD<Long, Long> categoryidRDD, // <clickCategoryId, clickCategoryId>、<orderCategoryId, orderCategoryId>、<payCategoryId, payCategoryId>
			JavaPairRDD<Long, Long> clickCategoryId2CountRDD, // <clickCategoryId, clickCategoryId_count>
			JavaPairRDD<Long, Long> orderCategoryId2CountRDD, // <orderCategoryId, orderCategoryId_count>
			JavaPairRDD<Long, Long> payCategoryId2CountRDD) { // <payCategoryId, payCategoryId_count>
		...
		/*
		 * <categoryid, str_val> leftJoin <orderCategoryId, orderCategoryId_count> =
		 * <categoryid, Tuple2<str_val, Optional<orderCategoryId_count>>>
		*/
		tmpMapRDD = tmpMapRDD.leftOuterJoin(orderCategoryId2CountRDD).mapToPair(
				new PairFunction<Tuple2<Long,Tuple2<String,Optional<Long>>>, Long, String>() {
					...
					public Tuple2<Long, String> call(Tuple2<Long, Tuple2<String, Optional<Long>>> tuple) throws Exception {
						...
						String value = tuple._2._1; // 上一方法结果
						Optional<Long> optional = tuple._2._2;
						long orderCount = 0L;
						if(optional.isPresent()) {
							orderCount = optional.get();
						}
						value = value + "|" + Constants.FIELD_ORDER_COUNT + "=" + orderCount; // 上一方法结果字符串拼接
						return new Tuple2<Long, String>(categoryid, value);
					}
				}
		);
		...
		/*
		 * <categoryid, str_val> leftJoin <payCategoryId, payCategoryId_count> =
		 * <categoryid, Tuple2<str_val, Optional<payCategoryId_count>>>
		*/
		tmpMapRDD = tmpMapRDD.leftOuterJoin(payCategoryId2CountRDD).mapToPair(
				new PairFunction<Tuple2<Long,Tuple2<String,Optional<Long>>>, Long, String>() {
					...
					public Tuple2<Long, String> call(Tuple2<Long, Tuple2<String, Optional<Long>>> tuple) throws Exception {
						...
						value = value + "|" + Constants.FIELD_PAY_COUNT + "=" + payCount; // 上一方法结果字符串拼接
						return new Tuple2<Long, String>(categoryid, value);
					}
				});
	...
	private static void getTop10Session(
			JavaSparkContext sc,
			final long taskid,
			List<Tuple2<CategorySortKey, String>> top10CategoryList, // <CategorySortKey, categoryid_click_count_str>
			JavaPairRDD<String, Row> sessionid2detailRDD) { // <sessionid, Row>
		/**
		 ...
		 */
		// 2.1、sessionid2detailRDD为：筛选→聚合→过滤→join明细 这里所以要再进行groupBy
		JavaPairRDD<String, Iterable<Row>> sessionid2detailsRDD = sessionid2detailRDD.groupByKey(); // <sessionid, Iterable<Row>>
		// Iterable<Tuple2<categoryid, (sessionid + "," + count)>>
		JavaPairRDD<Long, String> categoryid2sessionCountRDD = sessionid2detailsRDD.flatMapToPair(
				new PairFlatMapFunction<Tuple2<String, Iterable<Row>>, Long, String>() {
					...
					public Iterable<Tuple2<Long, String>> call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
						...
						// 返回 Iterable<Tuple2<categoryid, (sessionid + "," + count)>> 格式
		...
		// 2.2、获取到to10热门品类，被各个session点击的次数
		// <categoryid, categoryid>  join  Iterable<Tuple2<categoryid, (sessionid + "," + count)>>  =
		// Tuple2<categoryid, Tuple2<categoryid, (sessionid + "," + count)>>
		JavaPairRDD<Long, String> top10CategorySessionCountRDD = top10CategoryIdRDD.join(categoryid2sessionCountRDD)
				.mapToPair( // <categoryid, (sessionid + "," + count)>
		 ...
		 */
		// <categoryid, (sessionid + "," + count)>  groupByKey()  ==  <categoryid, Iterable<(sessionid + "," + count)>>
		JavaPairRDD<Long, Iterable<String>> top10CategorySessionCountsRDD = top10CategorySessionCountRDD.groupByKey();
		// <categoryid, Iterable<(sessionid + "," + count)>>  flatMapToPair  ==
		// Iterable<sessionid, sessionid>
		 ...
		 */
		// Iterable<sessionid, sessionid>  join  <sessionid, Row>  ==  <sessionid, Tuple2<sessionid, Row>>
